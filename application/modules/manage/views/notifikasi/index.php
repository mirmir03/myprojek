...
<head>
    ...
    <title>Senarai Notifikasi Pesanan</title>
    
    <!-- jQuery -->
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    
    <!-- Bootstrap CSS (assuming it's already loaded) -->
    <!-- DataTables CSS -->
    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            updateSidebarBadge(response.count);
                            showToast('Status berjaya dikemaskini', 'success');
                        } else {
                            showToast(response.message || 'Ralat tidak diketahui', 'error');
                        }
                    },
                    error: function () {
                        showToast('Ralat pelayan semasa mengemaskini status', 'error');
                    },
                    complete: function () {
            ...
            function updateNotificationCount() {
                $.ajax({
                    url: '<?= base_url("manage/notifikasi/get_notification_count") ?>',
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if (response.success) {
            ...
            function updateSidebarBadge(count) {
                ...
                if (count > 0) {
                    if (badge.length) {
                        ...
                        newBadge.addClass('pulse-animation');
                        setTimeout(() => newBadge.removeClass('pulse-animation'), 1500);
                    }
                } else {
                    if (badge.length) {
