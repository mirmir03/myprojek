...
class Template
{
	...
	{
		if(is_null($view)) {
			$nm_module = $this->_ci->router->fetch_module();
			$nm_controller = $this->_ci->router->fetch_class();
			$nm_function = $this->_ci->router->fetch_method();
						
			$view = $nm_controller . "/" .$nm_function;
			
			if(!file_exists("views/".$nm_controller)) {
				//show_error('Tiada folder <b>'.$nm_controller.'</b> dalam folder <b>views</b>. Sila create folder <b>'.$nm_controller.'</b> dulu !!  ');
			}

			if(!file_exists("gen/views/".$view.".php")) {
				//show_error('Tiada file <b>'.$nm_function.'.php</b> dalam folder <b>views/'.$nm_controller.'</b>');
			}
		} 
	...
	*/
    ...
    {
        if (empty($tmessage) && (class_exists('CI_Session', false) || class_exists('Session',false))) {
            return;
        }
